...
def resetSBaseMouseCam(base) :
    mat = p3d.Mat4(base.cam.getMat())
    mat.invertInPlace()
...
class SurroundView(ShowBase):
    def __init__(self):
        ...
        self.boat = self.loader.loadModel("./Raymarine_modeling.glb")
        #self.boat.setHpr(0, -90, 0)

        bbox = self.boat.getTightBounds()
        boatcenter = (bbox[1] + bbox[0]) * 0.5
        loadscale = bbox[1] - bbox[0]
        self.boat.setPos(-boatcenter.x, -boatcenter.y, -boatcenter.z)
        #this.shiplength = 12.8
        #this.shipbreadth = 4.2
        #this.shipheight = 6.2
        #this.shipelevation = 3.1
        self.boat.setScale(1280 / loadscale.x, 620 / loadscale.y, 420 / loadscale.z)
        self.boat.setHpr(-90, -90, 0)
        self.boat.setTwoSided(True)
        ...
        self.plight = p3d.PointLight('plight')
        self.plight.setColor(p3d.Vec4(1, 1, 1, 1))  # White light
        self.plightNP = self.renderObj.attachNewNode(self.plight)
        # Position the light
        self.plightNP.setPos(0, 0, -6000)
        #self.boat.setLight(self.plightNP)

        resetSBaseMouseCam(self)
...
def LoadImages(fileName) :
    img = cv.imread("./module/" + fileName)
    img = cv.resize(img, (img_width, img_height))
    img = cv.cvtColor(img, cv.COLOR_BGR2BGRA, )
    
    imgSeg = cv.imread("./module/" + "semantic_" + fileName)
    imgSeg = cv.resize(imgSeg, (img_width, img_height))
    imgSeg = cv.cvtColor(imgSeg, cv.COLOR_BGR2GRAY )
...
if __name__ == "__main__":
    ...
    front, frontSeg = LoadImages("front.png")
    right, rightSeg = LoadImages("right.png")
    rear, rearSeg = LoadImages("rear.png")
    left, leftSeg = LoadImages("left.png")
    
    imgnpArray = np.array([front, right, rear, left]).astype(np.uint8)
    segnpArray = np.array([frontSeg, rightSeg, rearSeg, leftSeg]).astype(np.uint32)
    imgArray = imgnpArray.reshape((4, 1920, 1080, 4))
    segArray = segnpArray.reshape((1, 1920, 1080, 4))
    cameraArray = imgArray[:, :, :, :].copy()
    semanticArray = segArray[:, :, :, :].copy()
