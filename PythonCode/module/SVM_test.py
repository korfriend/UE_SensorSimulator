...
class SurroundView(ShowBase):
    def __init__(self):
        ...
        def GeneratePlaneNode(svmBase):
            ...
            zeros = np.ones((4, 1280, 720), dtype=np.int32)
...
def InitSVM(base,
            ...
            imageWidth, imageHeight, imgs, worldpointlist, semanticArray):
    ...
    sensor_pos_array = [
            p3d.Vec3(645, 0, 216 - center_z), 
            p3d.Vec3(-400, 0, 216 - center_z), 
            p3d.Vec3(-155, 140, 216 - center_z), 
            p3d.Vec3(-155, -140, 216 - center_z)
            ]
    sensor_rot_z_array = [0, 90, 180, -90]
...
def ProcSvmFromPackets(base,
    ...
    imgs = np.array(EH_img)
    base.planeTexArray.setRamImage(imgs)

    semanticArray = np.array(semanticArray)
...
def PacketProcessing(packetInit: dict, q: queue):
    ...
    cv.imshow("0", segImg[0])
    cv.imshow("1", segImg[1])
    cv.imshow("2", segImg[2])
    cv.imshow("3", segImg[3])
    ...
    ProcSvmFromPackets(mySvm, 
                    ...
                    packetInit['imageWidth'], packetInit["imageHeight"], imgs, worldpointList, segImg)
