...
class SurroundView(ShowBase):
    def __init__(self):
        ...
        self.buffer1.setClearColor(Vec4(0, 0, 0, 1))
        self.buffer1.setSort(0)

        # Set up a camera for the first pass
        self.cam1 = self.makeCamera(self.buffer1, scene=foreground_render, lens=self.cam.node().getLens())
        self.cam1.reparentTo(self.cam)

        # Set up a buffer for the second pass
        self.buffer2 = self.win.makeTextureBuffer("buffer2", 1024, 1024)
        self.buffer2.setClearColor(Vec4(0, 0, 0, 1))
        self.buffer2.setSort(1)

        # Set up a camera for the second pass
        self.cam2 = self.makeCamera(self.buffer2, scene=background_render, lens=self.cam.node().getLens())
        ...
        self.boat.reparentTo(foreground_render)
        self.boat.setTag("FgScene", "True")
        #self.boat.setTag("BgScene", "False")

        self.axis = self.loader.loadModel('zup-axis')
        self.axis.setPos(0, 0, 0)
        self.axis.setScale(100)
        self.axis.reparentTo(foreground_render)
        self.axis.setTag("FgScene", "True")
        #self.axis.setTag("BgScene", "False")

        '''
        # Set up the final output
        self.finalOutput = self.win.getDisplayRegion(0)
        self.finalOutput.setSort(0)
        self.finalOutput.setClearColor(Vec4(0, 0, 0, 1))

        # Set up the post-processing shader
        self.postProcessShader = Shader.load(Shader.SL_GLSL, vertex="post1_vs.glsl", fragment="post1_ps.glsl")
        
        self.postProcessShaderNode = NodePath(
            PandaNode("post_process_shader_node"))
        self.postProcessShaderNode.setShader(self.postProcessShader)

        # Set up the scene to render in the final pass
        self.finalScene = NodePath(PandaNode("final_scene"))
        self.finalScene.reparentTo(self.cam)
        self.finalScene.setTexture(self.buffer1.getTexture())
        self.finalScene.setColor(Vec4(1, 1, 1, 1))
        self.finalScene.setShaderInput("tex2", self.buffer2.getTexture())
        self.finalScene.setShaderInput("tex3", self.buffer1.getTexture())
        self.finalScene.setShaderInput("tex4", self.buffer2.getTexture())
        self.finalScene.setShaderInput(
            "postProcessShader", self.postProcessShaderNode)
        
        # Set up the final camera
        self.finalCamNode = Camera("final_cam")
        self.finalCamLens = PerspectiveLens()
        self.finalCamNode.setLens(self.finalCamLens)
        self.finalCam = self.finalScene.attachNewNode(self.finalCamNode)

        # Set up the final display region
        self.finalOutput.setCamera(self.finalCam)
        '''

        # Set up the task to update the post-processing shader
        self.taskMgr.add(self.updatePostProcessShader,
        ...
        def GeneratePlaneNode(svmBase):
            ...
            svmBase.planeShader = Shader.load(
                Shader.SL_GLSL, vertex="svm_vs.glsl", fragment="svm_ps2.glsl")
            ...
            for i in range(4):
                svmBase.plane.setShaderInput(
                    'matViewProj' + str(i), p3d.Mat4())
            ...
            svmBase.semanticTexArray.setup2dTextureArray(
                256, 256, 4, p3d.Texture.T_unsigned_byte, p3d.Texture.F_rgba)
            svmBase.plane.setShaderInput(
                'semanticImgs', svmBase.semanticTexArray)

        GeneratePlaneNode(self)
        #self.plane.setTag("FgScene", "False")
        self.plane.setTag("BgScene", "True")
        self.plane.reparentTo(background_render)
        self.accept('r', self.shaderRecompile)

        
        #foreground_render.setShader(testSahder())
        self.cam1.node().setInitialState(foreground_render.getState())
        self.cam1.node().setTagStateKey("FgScene")
        self.cam1.node().setTagState("True", foreground_render.getState())

        #background_render.setShader(self.planeShader)
        self.cam2.node().setInitialState(background_render.getState())
        self.cam2.node().setTagStateKey("BgScene")
        self.cam2.node().setTagState("True", background_render.getState())
    ...
    def updatePostProcessShader(self, task):
        time = task.time
        #self.postProcessShaderNode.setShaderInput("time", time)
        return task.cont
...
tex = p3d.Texture()
tex.setup2dTexture(
    width, height, p3d.Texture.T_unsigned_byte, p3d.Texture.F_rgba)
    ...
    while(True):
        ...
        if index == 0xffffffff:
            ...
            if mySvm.isInitializedUDP == True:
                ...
                verFoV = 2 * math.atan(math.tan(150 * np.deg2rad(1) / 2)
                                       * (imageHeight/imageWidth)) * np.rad2deg(1)
                projMat = createOglProjMatrix(
                    verFoV, imageWidth/imageHeight, 10, 100000)
                ...
                # LHS, RHS same
                localCamMat = p3d.LMatrix4f.rotateMat(cam_rot_y, p3d.Vec3(
                    0, -1, 0)) * p3d.LMatrix4f.translateMat(cam_pos)
                sensorMatLHS_array = [
                    p3d.LMatrix4f(), p3d.LMatrix4f(), p3d.LMatrix4f(), p3d.LMatrix4f()]
                imgIdx = 0
                ...
                matViewProjs = [p3d.LMatrix4f(), p3d.LMatrix4f(),
                                p3d.LMatrix4f(), p3d.LMatrix4f()]
                for deg, pos in zip(sensor_rot_z_array, sensor_pos_array):
                    sensorMatRHS = p3d.LMatrix4f.rotateMat(deg, p3d.Vec3(
                        0, 0, -1)) * p3d.LMatrix4f.translateMat(pos.x, -pos.y, pos.z)
                    sensorMatLHS_array[imgIdx] = p3d.LMatrix4f.rotateMat(
                        deg, p3d.Vec3(0, 0, 1)) * p3d.LMatrix4f.translateMat(pos.x, pos.y, pos.z)
                    ...
                    if imgIdx == 1:
                        print(("camPos1 {}").format(
                            camMat.xformPoint(p3d.Vec3(0, 0, 0))))
                        print(("camPos2 {}").format(pos))
                        print(("camDir {}").format(
                            camMat.xform(p3d.Vec3(1, 0, 0))))
                        print(("camUp  {}").format(
                            camMat.xform(p3d.Vec3(0, 0, 1))))
                        print("############")
                    ...
                    mySvm.plane.setShaderInput(
                        "matViewProj" + str(imgIdx), viewProjMat)
                    imgIdx += 1
                ...
                mySvm.planeTexArray.setup2dTextureArray(
                ...
                mySvm.semanticTexArray.setup2dTextureArray(
                    imageWidth, imageHeight, 4, p3d.Texture.T_unsigned_byte, p3d.Texture.F_rgba)
                mySvm.plane.setShaderInput(
                    'semanticImgs', mySvm.semanticTexArray)
            ...
            if mySvm.isPointCloudSetup == True:
                ...
                offsetPoints = 0
                for i in range(4):
                    numSingleLidarPoints = int.from_bytes(
                        fullPackets[offsetPoints: 4 + offsetPoints], "little")
                    ...
                    for j in range(numSingleLidarPoints):
                        pX = struct.unpack(
                            '<f', fullPackets[0 + offsetPoints: 4 + offsetPoints])[0]
                        pY = struct.unpack(
                            '<f', fullPackets[4 + offsetPoints: 8 + offsetPoints])[0]
                        pZ = struct.unpack(
                            '<f', fullPackets[8 + offsetPoints: 12 + offsetPoints])[0]
                        #cR = np.frombuffer(fullPackets[12 + offsetPoints, 13 + offsetPoints], dtype=np.int8)[0]
                        cR = int.from_bytes(
                            fullPackets[12 + offsetPoints: 13 + offsetPoints], "little")
                        cG = int.from_bytes(
                            fullPackets[13 + offsetPoints: 14 + offsetPoints], "little")
                        cB = int.from_bytes(
                            fullPackets[14 + offsetPoints: 15 + offsetPoints], "little")
                        cA = int.from_bytes(
                            fullPackets[15 + offsetPoints: 16 + offsetPoints], "little")
                        #if j == 17 :
                        ...
                        mySvm.pointsColor.setData4f(
                            cB / 255.0, cG / 255.0, cR / 255.0, cA / 255.0)
